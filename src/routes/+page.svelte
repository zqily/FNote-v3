...
>
	<!-- Glow effect (positioned behind content) -->
	<div
		class="absolute inset-0 z-0 rounded-lg transition-opacity duration-1000 ease-in-out pointer-events-none"
		class:opacity-100={pb.isPlaying}
		class:opacity-0={!pb.isPlaying}
		style="background: radial-gradient(circle at center, rgba(34, 197, 94, 0.25) 0%, transparent 100%);"
	/>

	<!-- Content Wrapper (positioned on top of glow) -->
		...
		<div class="w-full max-w-xl mt-6 space-y-3">
			...
			<div class="w-full flex items-center space-x-3 text-xs text-zinc-400">
				...
				<input
					...
					class="progress-bar w-full h-1 bg-zinc-600 rounded-full appearance-none cursor-pointer accent-accent"
				/>
